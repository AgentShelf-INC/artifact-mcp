...
function normalizeAnchor(anchor) {
  if (anchor == null) return { anchor_path: null, anchor_x: null, anchor_y: null, anchor_w: null, anchor_h: null, anchor_approx: 0 };
...
export function createFeedbackStore({ db: database }) {
  const insertStmt = database.prepare(`
    INSERT INTO feedback (id, artifact_id, org, viewer_email, body, artifact_revision, parent_id, anchor_path, anchor_x, anchor_y, anchor_w, anchor_h, anchor_approx, anchor_page)
    VALUES (@id, @artifact_id, @org, @viewer_email, @body, @artifact_revision, @parent_id, @anchor_path, @anchor_x, @anchor_y, @anchor_w, @anchor_h, @anchor_approx, @anchor_page)
  `);
  ...
  function listForClient(clientId, artifactId, org) {
    if (org == null) return artifactId ? byClientArtifactStmt.all(clientId, artifactId) : byClientStmt.all(clientId);
    return artifactId ? byClientArtifactOrgStmt.all(clientId, artifactId, org) : byClientOrgStmt.all(clientId, org);
  }
