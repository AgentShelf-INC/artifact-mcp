...
// Admin-only Settings: manage organizations (name + explicit emails + domains + categories)
...
function keyRow(k, colors = {}) {
  ...
  return `<tr data-id="${esc(k.client_id)}" class="${revoked ? "revoked" : ""}">
    ...
    <td data-label="Organization"><span class="org-tag" data-org="${esc(k.org)}" style="--org-k:${orgColor(k.org, colors[k.org])};--org-accent:color-mix(in oklab,var(--org-k) 72%,var(--ink))"><span class="org-keyline"></span>${esc(k.org)}</span></td>
...
function orgCard(o) {
  ...
  // The color picker seeds from the stored hex if set, else the derived color as a starting point.
  const swatch = /^#[0-9a-fA-F]{6}$/.test(o.color || "") ? o.color : "#356b9f";
  return `<article class="org-card" data-org="${esc(o.name)}" data-color="${esc(o.color || "")}">
    <header class="org-card-head">
      <span class="org-tag" data-org="${esc(o.name)}" style="--org-k:${orgColor(o.name, o.color)};--org-accent:color-mix(in oklab,var(--org-k) 72%,var(--ink))"><span class="org-keyline"></span>${esc(o.name)}</span>
    ...
    <div class="org-row"><span class="org-row-label">Specific emails</span>${chipRow("email", o.emails, "add email")}</div>
    <div class="org-row"><span class="org-row-label">Categories</span>${chipRow("category", o.categories, "add category")}</div>
    <div class="org-row"><span class="org-row-label">Color</span>
      <form class="org-color">
        <input type="color" class="oc-swatch" value="${esc(swatch)}" aria-label="Pick ${esc(o.name)} color">
        <input type="text" class="oc-hex" value="${esc(o.color || "")}" placeholder="auto (#RRGGBB)" maxlength="7" spellcheck="false" aria-label="Hex color for ${esc(o.name)}">
        <button type="submit" class="oc-save">Save</button>
        <button type="button" class="oc-reset" title="Use an auto color derived from the name">Auto</button>
      </form>
    </div>
...
export function renderSettings(viewer, keys, orgList) {
  const orgColors = Object.fromEntries((orgList || []).map((o) => [o.name, o.color]));
  ...
  const rows = keys.length
    ? keys.map((k) => keyRow(k, orgColors)).join("")
  ...
  const orgCards = orgList.length
    ...
    : `<p class="lede org-empty">No organizations yet. Create the first one below, then route authenticated viewers with domains or explicit email addresses.</p>`;
...
.org-tag{font:700 .68rem var(--font-mono);display:inline-flex;align-items:center;gap:.4rem;color:var(--org-accent)}.org-keyline{width:2px;height:1rem;background:currentColor}
...
.org-color{display:flex;align-items:center;gap:.5rem;flex-wrap:wrap}.oc-swatch{width:2.1rem;height:1.9rem;padding:0;border:1px solid var(--line);border-radius:5px;background:none;cursor:pointer}.oc-hex{width:6.5rem;font:.74rem var(--font-mono);background:var(--ground);border:1px solid var(--line);border-radius:5px;padding:.4rem .5rem;color:var(--ink)}.oc-hex:focus{border-color:var(--brass);outline:0}.oc-save,.oc-reset{font:700 .64rem var(--font-mono);border-radius:5px;padding:.4rem .6rem;cursor:pointer}.oc-save{border:1px solid var(--ink);background:var(--ink);color:var(--card)}.oc-save:hover{background:var(--brass);border-color:var(--brass)}.oc-reset{border:1px solid var(--line);background:var(--card);color:var(--ink-3)}.oc-reset:hover{border-color:var(--brass);color:var(--brass)}
...
@media(max-width:680px){.settings-hero{padding:2.6rem 0 1.8rem}.settings-hero h1{font-size:3rem}.settings-stats{width:100%}.settings-stat{flex:1;min-width:0}.settings-section{grid-template-columns:1fr;gap:1rem;padding:2rem 0}.section-index{font-size:1.5rem;display:flex;align-items:baseline;gap:.55rem}.section-index span{display:inline}.form-row{grid-template-columns:1fr}.form-row .btn{grid-column:auto;width:100%}.key-form{padding:.9rem}.reveal-head{display:block}.once{display:inline-block;margin-top:.65rem}.secretbox{flex-direction:column}.copy{min-height:2.35rem}.org-row{grid-template-columns:1fr;gap:.4rem}.org-new-row{grid-template-columns:1fr}.org-new-row .btn{grid-column:auto;width:100%}.webhook-add{grid-template-columns:1fr}.wh-preselect{grid-column:auto}
  .table-wrap{overflow:visible}.registry{border-top-width:1px}thead{display:none}tbody{display:grid;gap:.7rem}tbody tr{display:grid;grid-template-columns:1fr 1fr;background:var(--card);border:1px solid var(--line);border-left:3px solid var(--brass);padding:.65rem;box-shadow:var(--shadow)}tbody td{display:block;border:0;padding:.42rem .45rem;min-width:0}tbody td::before{content:attr(data-label);display:block;font:700 .53rem var(--font-mono);text-transform:uppercase;letter-spacing:.07em;color:var(--ink-3);margin-bottom:.22rem}td.name{display:flex;align-items:center;grid-column:1/-1}td.name::before{display:none}td.right{display:flex;justify-content:flex-end;align-items:end}.revoke{min-height:2rem}.empty-row{display:block}.empty-row td{display:block}.empty-row td::before{display:none}
}
...
<div class="wrap">
  <header class="masthead">
    <a class="brand" href="/" aria-label="Back to Artifact Index"><span class="brand-mark">A</span><span><strong>Artifact Index</strong><small>${esc(SITE_HOST)}</small></span></a>
    <nav class="header-actions" aria-label="Account">
      ...
      <span class="identity" style="--identity-k:${orgColor("admin")};--identity-accent:color-mix(in oklab,var(--identity-k) 72%,var(--ink))"><span class="identity-dot"></span><span class="identity-email">${esc(viewer.email)}</span><strong>Admin</strong></span>
  ...
  <main class="settings-main" id="settings-content">
    <section class="settings-hero">
      <div><p class="eyebrow">Administration &middot; tenants &amp; access</p><h1>Organizations &amp; <em>Keys</em></h1><p class="settings-copy">Define each organization, the domains or explicit email addresses that route their team into it, and the categories they publish under &mdash; then issue the credentials their agents use to publish.</p></div>
    ...
    <section class="settings-section" aria-labelledby="orgs-title">
      ...
      <div class="settings-panel">
        <h2 id="orgs-title">Organizations, people, domains &amp; categories</h2>
        <p class="lede">Registered domains route authenticated viewers into an org. Specific emails override domain routing for contractors or shared providers, but the user must also be permitted by the operator's Cloudflare Access Allow policy. Adding an address here does not invite or authenticate them. Categories are the folders their artifacts can be grouped under.</p>
        <p class="lede">Webhook URLs are always masked here and in HTTP responses. They are encrypted at rest when <code>WEBHOOK_ENC_KEY</code> is configured; otherwise the documented plaintext fallback applies.</p>
        <div class="org-list" id="org-list">${orgCards}</div>
        <form class="org-new" id="org-new">
          <div class="org-new-row">
            <div class="field"><label for="o-name">Org name</label><input id="o-name" placeholder="acme" autocomplete="off" spellcheck="false" required><small>Lowercase id used everywhere</small></div>
            <div class="field"><label for="o-domain">Email domain</label><input id="o-domain" placeholder="acme.example" autocomplete="off" spellcheck="false"><small>Optional &mdash; add more later</small></div>
            <div class="field"><label for="o-label">Label</label><input id="o-label" placeholder="Acme" autocomplete="off"><small>Optional display name</small></div>
    ...
    <section class="settings-section" aria-labelledby="issue-title">
      ...
      <div class="settings-panel">
        ...
        <form class="key-form" id="key-form">
          <div class="form-row">
            <div class="field"><label for="name">Key name</label><input id="name" placeholder="alice-acme" autocomplete="off" required><small>Stable machine-readable ID</small></div>
  ...
  <footer class="footer"><span>Artifact Index · Administration</span><span>Upload keys are stored hashed</span></footer>
</div>
...
(function(){
  ...
  function orgColor(name){var value=String(name||''),hash=2166136261;for(var i=0;i<value.length;i++){hash^=value.charCodeAt(i);hash=Math.imul(hash,16777619);}return 'hsl('+((hash>>>0)%360)+' 68% 52%)';}
  ...
  function orgCardEl(o){
    ...
    art.innerHTML='<header class="org-card-head"><span class="org-tag" data-org="'+safe(o.name)+'" style="--org-k:'+orgColor(o.name)+';--org-accent:color-mix(in oklab,var(--org-k) 72%,var(--ink))"><span class="org-keyline"></span>'+safe(o.name)+'</span>'+(o.label?'<span class="org-label">'+safe(o.label)+'</span>':'')+'<span class="org-meta">'+(o.keyCount||0)+' keys</span><button class="org-del" type="button">Delete</button></header>'
      ...
      +'<div class="org-row"><span class="org-row-label">Specific emails</span><div class="chips" data-kind="email"><form class="chip-add" data-kind="email"><input placeholder="add email" autocomplete="off" spellcheck="false"><button type="submit" aria-label="Add">+</button></form></div></div>'
    ...
    ['domain','email','category'].forEach(function(kind){
      var add=art.querySelector('.chips[data-kind="'+kind+'"] .chip-add');
      (kind==='domain'?(o.domains||[]):kind==='email'?(o.emails||[]):(o.categories||[])).forEach(function(v){add.parentNode.insertBefore(chipEl(kind,v),add);});
    });
    return art;
  }
  ...
  if(orgList)orgList.addEventListener('submit',function(e){
    ...
    if(!val)return;
    var url='/settings/orgs/'+encodeURIComponent(org)+(kind==='domain'?'/domains':kind==='email'?'/emails':'/categories');
    var body=kind==='domain'?{domain:val}:kind==='email'?{email:val}:{name:val};
    ...
    .then(function(res){btn.disabled=false;
      ...
      f.parentNode.insertBefore(chipEl(kind,kind==='domain'?res.d.domain:kind==='email'?res.d.email:res.d.name),f);input.value='';input.focus();
  ...
  if(orgList)orgList.addEventListener('click',function(e){
    ...
    var reset=e.target.closest('.oc-reset');
    if(reset){var rcard=reset.closest('.org-card');reset.disabled=true;saveColor(rcard,rcard.dataset.org,'',reset);return;}
    ...
    if(x){
      ...
      else if(kind==='email'){url+='/emails/'+encodeURIComponent(val);}
      else{url+='/categories';opt.headers={'content-type':'application/json'};opt.body=JSON.stringify({name:val});}
      x.disabled=true;
      fetch(url,opt).then(function(r){return r.json().then(function(d){return {ok:r.ok,d:d};});}).then(function(res){var d=res.d;
        if(res.ok&&(kind==='email'||d.removed!==false))chip.remove();else{x.disabled=false;alert(d.error||'Could not remove');}
    ...
    if(del){
      ...
      if(!confirm('Delete organization "'+org2+'"? Its email/domain mappings and category list are removed. Existing artifacts and keys are NOT deleted.'))return;
      del.disabled=true;
  ...
  // --- Org accent color picker ---
  function ocDerive(name){var v=String(name||''),hash=2166136261;for(var i=0;i<v.length;i++){hash^=v.charCodeAt(i);hash=Math.imul(hash,16777619);}return 'hsl('+((hash>>>0)%360)+' 68% 52%)';}
  function applyOrgColor(card,color){card.dataset.color=color||'';var tag=card.querySelector('.org-tag');if(tag)tag.style.setProperty('--org-k',color||ocDerive(card.dataset.org));var hex=card.querySelector('.oc-hex');if(hex)hex.value=color||'';}
  function saveColor(card,org,hex,btn){
    fetch('/settings/orgs/'+encodeURIComponent(org)+'/color',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({color:hex})})
      .then(function(r){return r.json().then(function(d){return {ok:r.ok,d:d};});})
      .then(function(res){if(btn)btn.disabled=false;if(!res.ok){alert(res.d.error||'Could not save color');return;}applyOrgColor(card,res.d.color);})
      .catch(function(){if(btn)btn.disabled=false;alert('Network error');});
  }
  if(orgList)orgList.addEventListener('input',function(e){
    var card=e.target.closest('.org-card');if(!card)return;
    if(e.target.classList.contains('oc-swatch')){var hx=card.querySelector('.oc-hex');if(hx)hx.value=e.target.value;}
    else if(e.target.classList.contains('oc-hex')){var v=e.target.value.trim();if(/^#[0-9a-fA-F]{6}$/.test(v)){var sw=card.querySelector('.oc-swatch');if(sw)sw.value=v;}}
  });
  if(orgList)orgList.addEventListener('submit',function(e){
    var form=e.target.closest('.org-color');if(!form)return;e.preventDefault();
    var card=form.closest('.org-card'),save=form.querySelector('.oc-save');save.disabled=true;
    saveColor(card,card.dataset.org,form.querySelector('.oc-hex').value.trim(),save);
  });
