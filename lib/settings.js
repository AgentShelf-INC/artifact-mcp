...
function keyRow(k) {
  ...
  return `<tr data-id="${esc(k.client_id)}" class="${revoked ? "revoked" : ""}">
    ...
    <td data-label="Organization"><span class="org-tag" data-org="${esc(k.org)}" style="--org-k:${orgColor(k.org)};--org-accent:color-mix(in oklab,var(--org-k) 72%,var(--ink))"><span class="org-keyline"></span>${esc(k.org)}</span></td>
...
function orgCard(o) {
  ...
  return `<article class="org-card" data-org="${esc(o.name)}">
    <header class="org-card-head">
      <span class="org-tag" data-org="${esc(o.name)}" style="--org-k:${orgColor(o.name)};--org-accent:color-mix(in oklab,var(--org-k) 72%,var(--ink))"><span class="org-keyline"></span>${esc(o.name)}</span>
