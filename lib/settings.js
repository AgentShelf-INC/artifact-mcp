...
export function renderSettings(viewer, keys, orgs) {
  const rows = keys.length
    ...
    : `<tr><td colspan="5" class="dim">No keys yet. Generate one below.</td></tr>`;
...
<div class="wrap">
  ...
  <section class="panel">
    ...
    <table>
      <thead><tr><th>Name</th><th>Org</th><th>Created</th><th>Status</th><th></th></tr></thead>
...
(function(){
  ...
  gen.addEventListener('click',function(){
    ...
    fetch('/settings/keys',{method:'POST',headers:{'content-type':'application/json'},
      body:JSON.stringify({clientId:name.value,org:org.value})})
    ...
    .then(function(res){
      ...
      tr.innerHTML='<td class="mono name">'+d.clientId+'</td><td><span class="org-tag" data-org="'+d.org+'">'+d.org+'</span></td><td class="mono dim">'+fmt(d.created_at)+'</td><td class="status"><span class="pill on">active</span></td><td class="right"><button class="revoke" type="button">Revoke</button></td>';
