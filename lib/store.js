...
export function createArtifactStore({
  ...
  function metadata({ id, clientId, org, uploaderLabel, title, description, bytes, isBundle, entry }) {
    return {
      id,
      client_id: clientId,
      org: org || "default",
  ...
  function publish({ clientId, org, uploaderLabel, html, title, description }) {
    ...
    try {
      ...
      insert.run(metadata({ id, clientId, org, uploaderLabel, title, description, bytes, isBundle: false }));
      inserted = true;
  ...
  function publishBundle({ clientId, org, uploaderLabel, files: bundleFiles, entry, title, description }) {
    ...
    try {
      ...
      insert.run(metadata({
        id, clientId, org, uploaderLabel, title, description, bytes: total, isBundle: true, entry: selectedEntry
      }));
      inserted = true;
  ...
  function update({ id, clientId, isAdmin = false, html, files: bundleFiles, entry, title, description }) {
    ...
    try {
      database.transaction(() => {
        updateMetaStmt.run({ id, title: nextTitle, description: nextDescription, bytes: nextBytes, entry: nextEntry });
        if (staged) {
    ...
    return { ok: true, id, revision: updated.revision, bytes: updated.bytes, is_bundle: !!updated.is_bundle, entry: updated.entry };
  }
