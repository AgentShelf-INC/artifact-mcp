...
export function createArtifactStore({
  ...
  const insert = database.prepare(`
    INSERT INTO artifacts (id, client_id, org, uploader_label, title, description, bytes, is_bundle, entry, category, body_sha256)
    VALUES (@id, @client_id, @org, @uploader_label, @title, @description, @bytes, @is_bundle, @entry, @category, @body_sha256)
  `);
  ...
  const listBodies = database.prepare("SELECT id, is_bundle, body_sha256 FROM artifacts");
  ...
  const insertRevisionStmt = database.prepare(`
    ...
    VALUES (@artifact_id, @org, @revision, @title, @description, @category, @bytes, @is_bundle, @entry, @body_sha256)
  `);
  ...
  const listRevisionsStmt = database.prepare(
    "SELECT revision, title, description, category, bytes, is_bundle, entry, body_sha256, created_at FROM artifact_revisions WHERE artifact_id = ? ORDER BY revision DESC"
  );
  ...
  function metadata({ id, clientId, org, uploaderLabel, title, description, bytes, isBundle, entry, category, bodySha256 }) {
    return {
      id,
      client_id: clientId,
      org: org || "default",
  ...
  function publish({ clientId, org, uploaderLabel, html, title, description, category }) {
    ...
    try {
      ...
      insert.run(metadata({ id, clientId, org, uploaderLabel, title, description, bytes, isBundle: false, category, bodySha256 }));
      inserted = true;
  ...
  function publishBundle({ clientId, org, uploaderLabel, files: bundleFiles, entry, title, description, category }) {
    ...
    try {
      ...
      insert.run(metadata({
        id, clientId, org, uploaderLabel, title, description, bytes: total, isBundle: true,
    ...
    for (const name of names) {
      ...
      if (name.startsWith(".") && (name.includes(".staging-") || name.includes(".trash-"))) {
        ...
        if (cleanTransient) {
          ...
          } else if (row && name.includes(".staging-") && row.body_sha256 && bodyDigestOnDisk(row.id, row.is_bundle) !== row.body_sha256) {
  ...
  return {
    ...
    moveArtifactToOrg,
    readBundleFile,
    listOrgIds: (org, { includeHidden = false } = {}) => (includeHidden ? listIdsByOrgIncludingHidden : listIdsByOrg).all(org).map((row) => row.id),
    readArtifact,
    listForClient: (clientId, org) => (org == null ? listByClient.all(clientId) : listByClientOrg.all(clientId, org)),
    listOrgGroupedByClient: (org) => groupBy(listByOrg.all(org), (row) => row.client_id),
    listOrgArtifacts: (org, { includeHidden = false } = {}) => (includeHidden ? listByOrgIncludingHidden : listByOrg).all(org),
    listAllGroupedByOrg: ({ includeHidden = false } = {}) => groupBy((includeHidden ? listAll : listAllVisible).all(), (row) => row.org),
