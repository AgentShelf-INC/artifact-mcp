...
function position(anchor){try{var id=String(anchor&&anchor.id||""),x=clamp(anchor&&anchor.x),y=clamp(anchor&&anchor.y),bw=clamp(anchor&&anchor.w),bh=clamp(anchor&&anchor.h),box=bw!==null&&bh!==null&&bw>0&&bh>0,el=null;if(anchor&&typeof anchor.path==="string"&&anchor.path){try{el=d.querySelector(anchor.path);}catch(_){}}var size=dimensions();if(box){if(x===null||y===null)return {id:id,lost:true};var bx=x*size.w-w.scrollX,by=y*size.h-w.scrollY,pw=Math.min(bw,1-x)*size.w,ph=Math.min(bh,1-y)*size.h;if(!Number.isFinite(bx)||!Number.isFinite(by)||!Number.isFinite(pw)||!Number.isFinite(ph)||pw<=0||ph<=0)return {id:id,lost:true};return {id:id,x:bx,y:by,w:pw,h:ph,lost:false};}if(el){var rect=el.getBoundingClientRect();return {id:id,x:rect.left+rect.width/2,y:rect.top+rect.height/2,lost:false};}if(x===null||y===null)return {id:id,lost:true};var px=x*size.w-w.scrollX,py=y*size.h-w.scrollY;if(!Number.isFinite(px)||!Number.isFinite(py))return {id:id,lost:true};return {id:id,x:px,y:py,lost:false};}catch(_){return {id:String(anchor&&anchor.id||""),lost:true};}}
...
function point(el){var rect=el.getBoundingClientRect(),size=dimensions();return {type:"anchor:picked",x:clamp((rect.left+rect.width/2+w.scrollX)/size.w),y:clamp((rect.top+rect.height/2+w.scrollY)/size.h),path:pathFor(el)};}
function up(ev){try{if(!drag||ev.pointerId!==drag.id)return;ev.preventDefault();ev.stopPropagation();var current=drag;if(current.moved){var size=dimensions(),left=Math.min(current.x,ev.clientX)+w.scrollX,top=Math.min(current.y,ev.clientY)+w.scrollY,bx=clamp(left/size.w),by=clamp(top/size.h),bw=Math.min(Math.abs(ev.clientX-current.x)/size.w,1-(bx===null?1:bx)),bh=Math.min(Math.abs(ev.clientY-current.y)/size.h,1-(by===null?1:by));if(bx!==null&&by!==null&&Number.isFinite(bw)&&Number.isFinite(bh)&&bw>0&&bh>0)post({type:"anchor:picked",x:bx,y:by,w:bw,h:bh,path:pathFor(current.el)});}else post(point(current.el));stopPicking();}catch(_){stopPicking();}}
