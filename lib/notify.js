...
async function deliver(row, event, payload, fetchImpl, preview) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 4000);
  try {
    const body = buildEmbed(event, payload);
    const hasPreview = Buffer.isBuffer(preview) && preview.length > 0 && ["published", "updated", "restored"].includes(event);
    let requestBody;
    let headers;
    if (hasPreview) {
      body.embeds[0].image = { url: "attachment://preview.png" };
      const form = new FormData();
      form.append("payload_json", JSON.stringify(body));
      form.append("files[0]", new Blob([preview], { type: "image/png" }), "preview.png");
      requestBody = form;
    } else {
...
export function emit(event, org, payload = {}, { fetchImpl = globalThis.fetch, preview } = {}) {
  try {
    const rows = webhooks.forEvent(org, event);
    for (const row of rows) void deliver(row, event, { ...payload, org }, fetchImpl, preview);
  } catch {}
}
