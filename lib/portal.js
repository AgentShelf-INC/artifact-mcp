...
// FNV-1a gives every organization ID a stable hue without exposing it in CSS.
// The middle lightness keeps the accent visible against both application themes.
  ...
  const chips = showChips
    ? `<div class="filters" id="filters" aria-label="Filter by organization">
        ...
        ${sections
          .map(
            (s) =>
              `<button class="chip" data-org="${esc(s.org)}" aria-pressed="false"><span class="dot" style="background:${orgColor(s.org)}"></span>${esc(s.org)} <span>${s.items.length}</span></button>`
          )
          .join("")}
      </div>`
    : "";
  ...
  const body =
    total === 0
      ...
      : sections
          ...
          .map(
            (s, index) => `
      <section class="org" data-org="${esc(s.org)}" style="--org-k:${orgColor(s.org)};--org-accent:color-mix(in oklab,var(--org-k) 72%,var(--ink))">
...
<div class="wrap">
  <header class="masthead">
    ...
    <nav class="header-actions" aria-label="Account">
      ...
      <span class="identity" style="--identity-k:${orgColor(viewer.isAdmin ? "admin" : viewer.org)};--identity-accent:color-mix(in oklab,var(--identity-k) 72%,var(--ink))"><span class="identity-dot"></span><span class="identity-email">${esc(viewer.email)}</span><strong>${esc(role)}</strong></span>
