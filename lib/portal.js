...
function card(a, fav) {
  ...
  <article class="card" data-id="${esc(a.id)}" data-org="${esc(a.org)}" data-fav="${fav ? 1 : 0}"
           ...
           style="--k:${hue}">
    <div class="preview">
      <iframe class="pv" src="/raw/${esc(a.id)}" sandbox="" scrolling="no" loading="lazy"
    ...
    <div class="label">
      ...
      <div class="actions">
        ...
        <a class="act dl" href="/raw/${esc(a.id)}?download" download title="Download HTML">&#8681;</a>
...
<body>
  <header class="vbar" style="--k:${hue}">
    ...
    <div class="vright">
      ...
      ${prev}${next}
      <a class="vopen" href="/raw/${esc(meta.id)}" target="_blank" rel="noopener" title="Open raw">&#8599;</a>
  ...
  <iframe class="vframe" src="/raw/${esc(meta.id)}" title="${esc(meta.title)}"
