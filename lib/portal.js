...
function card(a, reaction = {}, aggregate = {}, showAggregate = false) {
  ...
  return `
  <article class="card" data-id="${esc(a.id)}" data-org="${esc(a.org)}" data-fav="${favorite ? 1 : 0}"
...
// (3 visible, most-recently-modified first, paged by arrows). ctx: { reactionFor, sentiment, isAdmin }.
function renderCategorySections(items, ctx) {
  ...
  return keys
    .map((key) => {
      ...
      const cards = rows.map((a) => card(a, ctx.reactionFor(a.id), ctx.sentiment.get(a.id) || {}, ctx.isAdmin)).join("");
      return `<div class="cat" data-cat="${esc(label.toLowerCase())}">
        <div class="cat-head">
...
export function renderGallery(viewer, sections, reactions = new Map(), sentiment = new Map()) {
  ...
  const body =
    total === 0
      ...
      <section class="org" data-org="${esc(s.org)}" style="--org-k:var(${orgVar(s.org)})">
        ...
        ${renderCategorySections(s.items, { reactionFor, sentiment, isAdmin: viewer.isAdmin })}
      </section>`
          )
          .join("");
...
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.card{transition:none;opacity:1;transform:none;animation:none}.card:hover,.card.settled:hover{transform:none}.preview .pv,.preview-skeleton{transition:none}.preview-skeleton::after{animation:none}}
`;
...
export function renderArtifactShell(meta, nav, reaction = { favorite: 0, vote: 0 }, feedback = []) {
...
<body>
  ...
  <script>
    (function(){
      ...
      function fbOpen(open){if(open){var hp=document.getElementById('vhist-panel');if(hp&&hp.classList.contains('open')){hp.classList.remove('open');hp.setAttribute('aria-hidden','true');var ht=document.getElementById('vhist-toggle');if(ht)ht.setAttribute('aria-expanded','false');}}fbPanel.classList.toggle('open',open);fbPanel.setAttribute('aria-hidden',open?'false':'true');fbToggle.setAttribute('aria-expanded',open?'true':'false');if(open){setTimeout(function(){fbBody.focus();},180);}}
...
(function(){
  ...
  document.addEventListener('click',function(e){
    var del=e.target.closest('.del'),yes=e.target.closest('.yes'),no=e.target.closest('.no');
    if(del){
