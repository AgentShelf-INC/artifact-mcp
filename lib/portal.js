  ...
  <main>
    <section class="intro" aria-labelledby="page-title">
      <div>
        ...
        <h1 id="page-title">${esc(APP_NAME)}</h1>
...
.preview .pv{position:absolute;inset:0;width:100%;height:100%;border:0;object-fit:cover;object-position:top center;pointer-events:none;background:#fff;opacity:0;transition:opacity .3s ease}
...
(function(){
  ...
  cards.forEach(function(c,i){
    ...
    if(frame){
      var reveal=function(){c.classList.add('preview-ready');};
      // A lazily-loaded or cached image may already be complete before this script runs, in
      // which case the 'load' event never fires; reveal synchronously in that case. Also reveal
      // on error so a failed request can't leave the skeleton spinning forever.
      if(frame.complete && frame.naturalWidth>0) reveal();
      else { frame.addEventListener('load',reveal,{once:true}); frame.addEventListener('error',reveal,{once:true}); }
    }
  });
