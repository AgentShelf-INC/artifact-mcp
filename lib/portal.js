...
export function renderArtifactShell(meta, nav, reaction = { favorite: 0, vote: 0 }, feedback = [], analytics = {}, viewer = {}) {
  ...
  const feedbackData = jsLiteral(JSON.stringify(feedback.map((f) => ({
    id: f.id, parent_id: f.parent_id, anchor_path: f.anchor_path, anchor_x: f.anchor_x, anchor_y: f.anchor_y,
...
<body>
  ...
  <script>
    (function(){
      ...
      function itemHtml(row,justNow){var resolved=!!row.resolved_at,anchored=row.anchor_x!=null&&row.anchor_y!=null,manage=canManage(row);return '<div class="vfb-item '+(resolved?'resolved':'')+'" data-id="'+fesc(row.id)+'"><div class="vfb-m"><span>'+fesc(row.viewer_email)+'</span><span>'+fesc(justNow?'Just now':'')+(resolved?' &middot; <span class="vfb-res">Resolved</span>':'')+'</span></div><div class="vfb-b">'+fesc(row.body)+'</div>'+(anchored?'<span class="vfb-anchor-state">Pinned comment</span>':'')+(manage?'<div class="vfb-manage"><button class="vfb-delete" type="button" data-feedback-action="delete">Delete</button>'+(resolved?'':'<button class="vfb-resolve" type="button" data-feedback-action="resolve">Resolve</button>')+'</div>':'')+'</div>';}
      ...
      feedbackRows.forEach(function(row){
        if(row.parent_id!=null||finiteFraction(row.anchor_x)===null||finiteFraction(row.anchor_y)===null)return;
      ...
      function paintPosition(pin,x,y,lost){if(lost){positionLost(pin);return;}var marker=markerFor(pin);marker.hidden=false;marker.style.left=Math.round(x)+'px';marker.style.top=Math.round(y)+'px';}
      ...
      function startAnchoredComment(anchor){var x=finiteFraction(anchor&&anchor.x),y=finiteFraction(anchor&&anchor.y);if(x===null||y===null)return;draftAnchor={x:x,y:y,path:typeof anchor.path==='string'?anchor.path.slice(0,512):undefined,approx:anchor.approx?1:0};setCommentMode(false);fbOpen(true);fbHint.classList.remove('error');fbHint.textContent=draftAnchor.approx?'Approximate pin selected.':'Pinned location selected.';}
      if(commentToggle)commentToggle.addEventListener('click',function(){setCommentMode(!commentMode);});
      overlay.addEventListener('click',function(e){if(!commentMode||bridgeReady)return;var rect=overlay.getBoundingClientRect();if(!rect.width||!rect.height)return;startAnchoredComment({x:(e.clientX-rect.left)/rect.width,y:(e.clientY-rect.top)/rect.height,approx:1});});
      ...
      window.addEventListener('message',function(event){
        ...
        data.anchors.slice(0,200).forEach(function(pos){if(!pos||typeof pos!=='object'||typeof pos.id!=='string')return;var pin=pinById[pos.id];if(!pin)return;if(pos.lost===true){paintPosition(pin,0,0,true);return;}if(typeof pos.x!=='number'||typeof pos.y!=='number'||!Number.isFinite(pos.x)||!Number.isFinite(pos.y))return;paintPosition(pin,pos.x,pos.y,false);});
      });
      ...
      function appendFeedback(row){
        ...
        if(!row.parent_id&&finiteFraction(row.anchor_x)!==null&&finiteFraction(row.anchor_y)!==null){var pin={id:String(row.id),path:typeof row.anchor_path==='string'?row.anchor_path.slice(0,512):null,x:finiteFraction(row.anchor_x),y:finiteFraction(row.anchor_y),approx:row.anchor_approx?1:0,stale:Number(row.artifact_revision)!==${Number(meta.revision) || 1}};pins.push(pin);pinById[pin.id]=pin;requestRepaint();}
