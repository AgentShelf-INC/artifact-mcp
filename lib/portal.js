...
// Cache-bust the preview/viewer iframes by content version. Raw responses carry
...
function card(a, reaction = {}, aggregate = {}, showAggregate = false, views = null, orgNames = [], categories = [], orgAccent = null) {
  ...
  const ver = bodyVersion(a);
  const verQ = ver ? `&v=${esc(ver)}` : "";
  const rawSrc = a.is_bundle ? `/raw/${esc(a.id)}/?preview${verQ}` : `/raw/${esc(a.id)}?preview${verQ}`;
