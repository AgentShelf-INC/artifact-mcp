  ...
  const body =
    total === 0
      ...
      <section class="org" data-org="${esc(s.org)}" style="--org-k:var(${orgVar(s.org)})">
        ...
        ${s.items.length ? renderCategorySections(s.items, { reactionFor, sentiment, isAdmin: viewer.isAdmin, viewCounts, orgNames }) : `<p class="org-empty"${viewer.isAdmin ? " data-drop-org=\"" + esc(s.org) + "\"" : ""}>No artifacts yet — drop one here to move it into ${esc(s.org)}.</p>`}
      </section>`
          )
          .join("");
