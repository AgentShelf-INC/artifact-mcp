  ...
  <script>
    (function(){
      ...
      function paintPosition(pin,x,y,lost){
        ...
        var marker=markerFor(pin);marker.hidden=!commentMode;marker.style.left=Math.round(x)+'px';marker.style.top=Math.round(y)+'px';
      }
      function setCommentMode(next){
        commentMode=!!next;commentToggle.setAttribute('aria-pressed',commentMode?'true':'false');
        [].slice.call(overlay.querySelectorAll('.vanchor-marker')).forEach(function(marker){marker.hidden=!commentMode;});
