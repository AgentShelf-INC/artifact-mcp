...
function card(a, reaction = {}, aggregate = {}, showAggregate = false, views = null) {
...
function renderCategorySections(items, ctx) {
  ...
  return keys
    .map((key) => {
      ...
      const cards = rows.map((a) => card(a, ctx.reactionFor(a.id), ctx.sentiment.get(a.id) || {}, ctx.isAdmin, ctx.viewCounts.get(a.id) || null)).join("");
  ...
  const body =
    total === 0
      ...
      <section class="org" data-org="${esc(s.org)}" style="--org-k:var(${orgVar(s.org)})">
        ...
        ${renderCategorySections(s.items, { reactionFor, sentiment, isAdmin: viewer.isAdmin, viewCounts })}
      </section>`
          )
          .join("");
