...
function card(a, reaction = {}, aggregate = {}, showAggregate = false, views = null, orgNames = [], categories = []) {
  ...
  return `
  <article class="card${a.hidden ? " is-hidden" : ""}" data-id="${esc(a.id)}" data-org="${esc(a.org)}" data-hidden="${a.hidden ? 1 : 0}" data-fav="${favorite ? 1 : 0}"${showAggregate ? " draggable=\"true\"" : ""}
...
function renderCategorySections(items, ctx) {
  ...
  return keys
    .map((key) => {
      ...
      const cards = rows.map((a) => card(a, ctx.reactionFor(a.id), ctx.sentiment.get(a.id) || {}, ctx.isAdmin, ctx.viewCounts.get(a.id) || null, ctx.orgNames, keys)).join("");
...
export function renderGallery(viewer, sections, reactions = new Map(), sentiment = new Map(), viewCounts = new Map(), topViewed = new Map()) {
  ...
  const chips = showChips
    ? `<div class="filters" id="filters" aria-label="Filter by organization">
        ...
        ${sections
          .map(
            (s) =>
              `<button class="chip" data-org="${esc(s.org)}" aria-pressed="false"><span class="dot" style="background:var(${orgVar(s.org)})"></span>${esc(s.org)} <span>${s.items.length}</span></button>`
          )
          .join("")}
      </div>`
    : "";
  ...
  const body =
    total === 0
      ...
      : sections
          ...
          .map(
            (s, index) => `
      <section class="org" data-org="${esc(s.org)}" style="--org-k:var(${orgVar(s.org)})">
        ...
        ${renderCategorySections(s.items, { reactionFor, sentiment, isAdmin: viewer.isAdmin, viewCounts, orgNames })}
      </section>`
          )
          .join("");
...
<meta name="color-scheme" content="light dark"><link rel="icon" href="${PORTAL_FAVICON}"><title>Artifacts &middot; neilblackman.dev</title>
...
<div class="wrap">
  <header class="masthead">
    <a class="brand" href="/" aria-label="Artifact Index home"><span class="brand-mark">A</span><span><strong>Artifact Index</strong><small>neilblackman.dev</small></span></a>
    <nav class="header-actions" aria-label="Account">
      ...
      <span class="identity" style="--identity-k:var(${orgVar(viewer.isAdmin ? "admin" : viewer.org)})"><span class="identity-dot"></span><span class="identity-email">${esc(viewer.email)}</span><strong>${esc(role)}</strong></span>
...
:root{
  ...
  --org-agentshelf:#356B9F;--org-trustedtech:#347455;--org-cairn:#8C6030;--org-admin:#66578B;
}
...
.identity strong{color:var(--identity-k);font-weight:700;text-transform:uppercase;letter-spacing:.04em;white-space:nowrap}
.identity-email{max-width:190px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.identity-dot{width:.48rem;height:.48rem;border-radius:50%;background:var(--identity-k);box-shadow:0 0 0 3px color-mix(in srgb,var(--identity-k) 15%,transparent);flex:none}
...
.org-head[data-drop-org]{outline-offset:4px}.org-head.drop-ready,.cat.drop-ready{outline:2px dashed var(--brass);outline-offset:5px;background:color-mix(in srgb,var(--brass-soft) 45%,transparent)}
.org-index{font:500 1.7rem/1 var(--font-display);color:var(--org-k);min-width:2rem}
.org-label{font-size:.58rem;margin:0 0 .18rem;color:var(--ink-3)}
.org-name{font:600 1rem/1.05 var(--font-mono);letter-spacing:.045em;text-transform:uppercase;color:var(--org-k);margin:0}
.org-rule{flex:1;height:1px;background:linear-gradient(90deg,var(--org-k),var(--line) 20%,var(--line))}
...
const SHELL_CSS = `
:root{color-scheme:light dark;--bg:#fff;--bar:#F3F5F6;--panel:#fff;--txt:#142235;--line:#CDD4DB;--dim:#607080;--brass:#9B681F;--danger:#9E3441;--positive:#2F7450;--negative:#A14C38;--org-agentshelf:#356B9F;--org-trustedtech:#347455;--org-cairn:#8C6030;--org-admin:#66578B}
@media(prefers-color-scheme:dark){:root{--bg:#0C1117;--bar:#151C24;--panel:#1D2630;--txt:#EBEEF1;--line:#303A45;--dim:#98A4AF;--brass:#D5A252;--danger:#DF7882;--positive:#72BB91;--negative:#D78670;--org-agentshelf:#6F9FD0;--org-trustedtech:#73B48C;--org-cairn:#CAA06A;--org-admin:#A194CD}}
:root[data-theme="light"]{color-scheme:light;--bg:#fff;--bar:#F3F5F6;--panel:#fff;--txt:#142235;--line:#CDD4DB;--dim:#607080;--brass:#9B681F;--danger:#9E3441;--positive:#2F7450;--negative:#A14C38;--org-agentshelf:#356B9F;--org-trustedtech:#347455;--org-cairn:#8C6030;--org-admin:#66578B}
:root[data-theme="dark"]{color-scheme:dark;--bg:#0C1117;--bar:#151C24;--panel:#1D2630;--txt:#EBEEF1;--line:#303A45;--dim:#98A4AF;--brass:#D5A252;--danger:#DF7882;--positive:#72BB91;--negative:#D78670;--org-agentshelf:#6F9FD0;--org-trustedtech:#73B48C;--org-cairn:#CAA06A;--org-admin:#A194CD}
...
function feedbackItem(f, currentRevision) {
  const resolved = !!f.resolved_at;
  const anchored = f.anchor_x != null && f.anchor_y != null;
  const state = anchored
    ? `<span class="vfb-anchor-state">${f.artifact_revision !== currentRevision ? `Placed on v${esc(f.artifact_revision)} · stale` : "Pinned comment"}</span>`
    : "";
  return `<div class="vfb-item ${resolved ? "resolved" : ""}" data-id="${esc(f.id)}">
    ...
    <div class="vfb-b">${esc(f.body)}</div>${state}
  </div>`;
}
...
export function renderArtifactShell(meta, nav, reaction = { favorite: 0, vote: 0 }, feedback = [], analytics = {}) {
  ...
  const feedbackHtml = feedback.length
    ? feedback.map((f) => feedbackItem(f, meta.revision)).join("")
  ...
  const feedbackData = jsLiteral(JSON.stringify(feedback.map((f) => ({
    id: f.id, anchor_path: f.anchor_path, anchor_x: f.anchor_x, anchor_y: f.anchor_y,
...
<body>
  ...
  <script>
    (function(){
      ...
      var frame=document.getElementById('vframe'),overlay=document.getElementById('vanchor-overlay'),commentToggle=document.getElementById('vcomment-toggle'),commentMode=false,bridgeReady=false,draftAnchor=null,bridgeTimer;
      var feedbackRows=JSON.parse(${feedbackData}),pins=[],pinById={},feedbackItems={};
      ...
      feedbackRows.forEach(function(row){
        if(finiteFraction(row.anchor_x)===null||finiteFraction(row.anchor_y)===null)return;
        var pin={id:String(row.id),path:typeof row.anchor_path==='string'?row.anchor_path.slice(0,512):null,x:finiteFraction(row.anchor_x),y:finiteFraction(row.anchor_y),approx:row.anchor_approx?1:0,stale:Number(row.artifact_revision)!==${Number(meta.revision) || 1}};
        pins.push(pin);pinById[pin.id]=pin;
      });
      ...
      function requestRepaint(){postToFrame('anchor:repaint',{anchors:pins.map(function(pin){return {id:pin.id,path:pin.path,x:pin.x,y:pin.y};})});}
      ...
      function markerFor(pin){
        ...
        marker=document.createElement('button');marker.type='button';marker.id='vanchor-'+pin.id;marker.className='vanchor-marker'+(pin.stale?' stale':'');marker.textContent=String(pinNumber(pin));marker.title=pin.stale?'Pinned comment '+pinNumber(pin)+' · placed on an older revision':'Pinned comment '+pinNumber(pin);
        marker.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();fbOpen(true);var item=feedbackItems[pin.id];if(item){item.classList.add('pin-focus');item.scrollIntoView({block:'center'});setTimeout(function(){item.classList.remove('pin-focus');},1600);}fbHint.textContent='Pinned comment '+pinNumber(pin)+(pin.stale?' · placed on an older revision.':'');});
      ...
      function positionLost(pin){
        ...
        var item=feedbackItems[pin.id];if(item&&!item.querySelector('.vfb-anchor-state[data-lost]')){var note=document.createElement('span');note.className='vfb-anchor-state';note.setAttribute('data-lost','1');note.textContent='Position lost · shown in this flat thread';item.appendChild(note);}
      }
      function paintPosition(pin,x,y,lost){
        if(lost){positionLost(pin);return;}
        var marker=markerFor(pin);marker.hidden=false;marker.style.left=Math.round(x)+'px';marker.style.top=Math.round(y)+'px';
      }
      function setCommentMode(next){
        commentMode=!!next;commentToggle.setAttribute('aria-pressed',commentMode?'true':'false');
        // Existing markers stay visible always — comment mode only governs PLACING new pins.
        document.body.classList.toggle('vpinning',commentMode);
        if(!commentMode){overlay.classList.remove('fallback');postToFrame('anchor:pick-off');return;}
        if(bridgeReady){overlay.classList.remove('fallback');postToFrame('anchor:pick-on');requestRepaint();}
        else overlay.classList.add('fallback');
      }
      function startAnchoredComment(anchor){
        var x=finiteFraction(anchor&&anchor.x),y=finiteFraction(anchor&&anchor.y);if(x===null||y===null)return;
        draftAnchor={x:x,y:y,path:typeof anchor.path==='string'?anchor.path.slice(0,512):undefined,approx:anchor.approx?1:0};
        setCommentMode(false);fbOpen(true);fbHint.classList.remove('error');fbHint.textContent=draftAnchor.approx?'Approximate pin selected.':'Pinned location selected.';
      }
      if(commentToggle)commentToggle.addEventListener('click',function(){setCommentMode(!commentMode);});
      overlay.addEventListener('click',function(e){
        if(!commentMode||bridgeReady)return;
        var rect=overlay.getBoundingClientRect();if(!rect.width||!rect.height)return;
        startAnchoredComment({x:(e.clientX-rect.left)/rect.width,y:(e.clientY-rect.top)/rect.height,approx:1});
      });
      ...
      window.addEventListener('message',function(event){
        ...
        if(data.type==='anchor:picked'){startAnchoredComment(data);return;}
        if(!Array.isArray(data.anchors))return;
        data.anchors.slice(0,200).forEach(function(pos){
          if(!pos||typeof pos!=='object'||typeof pos.id!=='string')return;var pin=pinById[pos.id];if(!pin)return;
          if(pos.lost===true){paintPosition(pin,0,0,true);return;}
          if(typeof pos.x!=='number'||typeof pos.y!=='number'||!Number.isFinite(pos.x)||!Number.isFinite(pos.y))return;
          paintPosition(pin,pos.x,pos.y,false);
        });
      });
      if(fbForm){fbForm.addEventListener('submit',function(e){
        e.preventDefault();
        var text=(fbBody.value||'').trim();
        if(!text){fbHint.textContent='Write something first.';fbHint.classList.add('error');return;}
        var btn=fbForm.querySelector('.vfb-send');btn.disabled=true;fbHint.classList.remove('error');fbHint.textContent='Sending…';
        fetch('/${esc(meta.id)}/feedback',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({body:text,anchor:draftAnchor||undefined})})
          .then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error(d.error||'Could not send feedback');return d;});})
          .then(function(d){
            var empty=fbList.querySelector('.vfb-empty');if(empty){empty.remove();}
            var item=document.createElement('div');item.className='vfb-item';item.setAttribute('data-id',d.id);
            item.innerHTML='<div class="vfb-m"><span>'+fesc(d.viewer_email)+'</span><span>Just now</span></div><div class="vfb-b">'+fesc(d.body)+'</div>';
            feedbackItems[d.id]=item;
            if(finiteFraction(d.anchor_x)!==null&&finiteFraction(d.anchor_y)!==null){var pin={id:String(d.id),path:typeof d.anchor_path==='string'?d.anchor_path.slice(0,512):null,x:finiteFraction(d.anchor_x),y:finiteFraction(d.anchor_y),approx:d.anchor_approx?1:0,stale:Number(d.artifact_revision)!==${Number(meta.revision) || 1}};pins.push(pin);pinById[pin.id]=pin;item.insertAdjacentHTML('beforeend','<span class="vfb-anchor-state">'+(pin.stale?'Placed on an older revision · stale':'Pinned comment')+'</span>');requestRepaint();}
            fbList.appendChild(item);fbList.scrollTop=fbList.scrollHeight;
            fbBody.value='';draftAnchor=null;fbHint.textContent='Sent to the author.';
            var n=(parseInt(fbCount.textContent,10)||0)+1;fbCount.textContent=n;fbCount.hidden=false;
          })
          .catch(function(err){fbHint.textContent=err.message||'Could not send feedback.';fbHint.classList.add('error');})
...
(function(){
  ...
  function requestMove(id,body,message){
    return fetch('/'+id+'/move',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)})
  ...
  document.querySelectorAll('.cat,[data-drop-org]').forEach(function(target){
    ...
    target.addEventListener('drop',function(e){if(!dragging)return;e.preventDefault();target.classList.remove('drop-ready');var c=dragging,id=c.dataset.id;
      ...
      else {var orgSection=target.closest('.org');var category=target.dataset.category;if(orgSection.dataset.org===c.dataset.org)requestMove(id,{category:category},'Artifact moved to '+(category||'Uncategorized'));}
    });
  });
})();
`;
