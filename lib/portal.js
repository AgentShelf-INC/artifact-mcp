...
function card(a, reaction = {}, aggregate = {}, showAggregate = false, views = null, orgNames = [], categories = [], orgAccent = null) {
  ...
  const rawSrc = a.is_bundle ? `/raw/${esc(a.id)}/?preview` : `/raw/${esc(a.id)}?preview`;
...
.notif-wrap{position:relative}.notif-toggle{position:relative}.notif-count{position:absolute;right:-.08rem;top:-.08rem;min-width:1rem;height:1rem;padding:0 .2rem;border-radius:.55rem;background:var(--danger);color:#fff;font:700 .55rem/1rem var(--font-mono);text-align:center}.notif-panel{position:absolute;z-index:30;right:0;top:calc(100% + .55rem);width:min(360px,calc(100vw - 2rem));max-height:min(480px,70vh);overflow:hidden;background:var(--paper);border:1px solid var(--line);border-radius:8px;box-shadow:0 18px 44px rgba(0,0,0,.22)}.notif-head{display:flex;align-items:center;justify-content:space-between;padding:.7rem .8rem;border-bottom:1px solid var(--line);font:700 .7rem var(--font-mono);text-transform:uppercase;letter-spacing:.05em}.notif-head button{border:0;background:transparent;color:var(--brass);font:700 .6rem var(--font-mono);cursor:pointer}.notif-list{max-height:410px;overflow-y:auto}.notif-row{display:flex;flex-direction:column;gap:.15rem;padding:.7rem .8rem;border-bottom:1px solid var(--line-2);text-decoration:none;background:var(--paper)}.notif-row:hover{background:var(--card)}.notif-row.unread{background:color-mix(in srgb,var(--brass) 8%,var(--paper));box-shadow:inset 3px 0 var(--brass)}.notif-meta{display:flex;justify-content:space-between;gap:.6rem;color:var(--ink-3);font:.58rem var(--font-mono)}.notif-meta strong{color:var(--ink-2);overflow:hidden;text-overflow:ellipsis}.notif-artifact{font:600 .78rem var(--font-display)}.notif-snippet{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--ink-2);font-size:.7rem}.notif-empty{padding:2rem 1rem;text-align:center;color:var(--ink-3);font-size:.78rem}
...
export function notSignedInPage() {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark"><link rel="icon" href="${PORTAL_FAVICON}"><title>Sign in &middot; Artifact Index</title>
<style>
:root{color-scheme:light dark;--bg:#E8EAED;--card:#fff;--ink:#142235;--dim:#506071;--line:#C8D0D8;--brass:#9B681F}
@media(prefers-color-scheme:dark){:root{--bg:#11161D;--card:#19212A;--ink:#EBEEF1;--dim:#A3ADB7;--line:#303A45;--brass:#D5A252}}
...
.mark{width:2.6rem;height:2.6rem;border-radius:8px;background:#142235;display:grid;place-items:center;flex:none}.mark svg{width:1.5rem;height:1.5rem}
...
<body><main class="si">
  <div class="brand"><span class="mark"><svg viewBox="0 0 64 64" aria-hidden="true"><path d="M18 47 30 15h5l12 32h-7l-3-9H27l-3 9Zm11-15h6l-3-10Z" fill="#D5A252"/></svg></span><span><strong>Artifact Index</strong><small>${esc(SITE_HOST)}</small></span></div>
  ...
  <script>
    (function(){
      ...
      function focusFeedback(id){
        ...
        var pin=pinById[id];if(pin){var marker=markerFor(pin);marker.classList.add('pin-focus');setTimeout(function(){marker.classList.remove('pin-focus');},2200);if(isBundle&&pin.page){frame.src=${jsLiteral(`/raw/${meta.id}/`)}+pin.page.split('/').map(encodeURIComponent).join('/')+'?anchor=1';}}
      }
...
const SCRIPT = `
(function(){
  var notifToggle=document.getElementById('notif-toggle'),notifPanel=document.getElementById('notif-panel'),notifSeen=document.getElementById('notif-seen'),notifCount=document.querySelector('.notif-count'),notifMarked=false;
  function markNotificationsSeen(){if(notifMarked)return;notifMarked=true;if(notifCount){notifCount.hidden=true;notifCount.textContent='0';}document.querySelectorAll('.notif-row.unread').forEach(function(row){row.classList.remove('unread');});fetch('/notifications/seen',{method:'POST'}).catch(function(){notifMarked=false;});}
  function openNotifications(open){if(!notifPanel)return;notifPanel.hidden=!open;notifToggle.setAttribute('aria-expanded',open?'true':'false');if(open)markNotificationsSeen();}
  if(notifToggle)notifToggle.addEventListener('click',function(e){e.stopPropagation();openNotifications(notifPanel.hidden);});
  if(notifSeen)notifSeen.addEventListener('click',function(){markNotificationsSeen();});
  document.addEventListener('click',function(e){if(notifPanel&&!notifPanel.hidden&&!e.target.closest('.notif-wrap'))openNotifications(false);});
  document.addEventListener('keydown',function(e){if(e.key==='Escape'&&notifPanel&&!notifPanel.hidden){openNotifications(false);notifToggle.focus();}});
