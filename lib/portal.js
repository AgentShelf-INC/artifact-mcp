...
.vframe{flex:1;min-height:0;width:100%;border:0;background:#fff}.vbar :focus-visible{outline:2px solid var(--brass);outline-offset:2px}
...
function feedbackItem(f) {
  ...
  return `<div class="vfb-item ${resolved ? "resolved" : ""}" data-id="${esc(f.id)}">
    ...
    <div class="vfb-b">${esc(f.body)}</div>
  </div>`;
}
  ...
  <script>
    (function(){
      ...
      if(fbForm){fbForm.addEventListener('submit',function(e){
        ...
        fetch('/${esc(meta.id)}/feedback',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({body:text})})
          ...
          .then(function(d){
            ...
            fbBody.value='';fbHint.textContent='Sent to the author.';
