    ...
    total === 0
      ...
      <section class="org" data-org="${esc(s.org)}" style="--org-k:var(${orgVar(s.org)})">
        ...
        <div class="grid">${[...s.items]
          ...
          .map((a) => card(a, reactionFor(a.id), sentiment.get(a.id) || {}, viewer.isAdmin))
          .join("")}</div>
      </section>`
          )
          .join("");
...
<body>
  <header class="vbar" style="--k:${hue}">
    ...
    <div class="vmid">
      ...
      <span class="vmeta"><span class="vorg">${esc(meta.org)}</span><span>·</span><span><span class="publisher-label">Published by </span>${esc(who)}</span><span class="vtype">${meta.is_bundle ? "Bundle" : "HTML"}</span></span>
    </div>
