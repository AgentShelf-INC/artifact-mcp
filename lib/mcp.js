...
import { publish, publishBundle, update, listForClient, remove, getArtifactMeta } from "./store.js";
import {
...
async function callTool(params, auth) {
  ...
  if (name === "update_artifact") {
    ...
    return toolResult({ id: result.id, url: urlFor(result.id), revision: result.revision, bytes: result.bytes, entry: result.entry });
  }
