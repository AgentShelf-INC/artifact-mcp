...
async function callTool(params, clientId) {
  ...
  if (name === "publish_artifact") {
    ...
    return toolResult({ id, url: urlFor(id), bytes });
  }
...
async function dispatch(msg, clientId) {
  switch (msg.method) {
    case "initialize":
      return {
    ...
    case "tools/call":
      return callTool(msg.params, clientId);
    default:
      throw Object.assign(new Error(`Method not found: ${msg.method}`), { rpcCode: -32601 });
  }
}

async function handleOne(msg, clientId) {
  ...
  try {
    const result = await dispatch(msg, clientId);
...
export async function handleMcp(payload, clientId) {
  if (Array.isArray(payload)) {
    if (payload.length === 0) return rpcError(null, -32600, "Invalid Request");
    const out = (await Promise.all(payload.map((m) => handleOne(m, clientId)))).filter(Boolean);
    return out.length ? out : null;
  }
  return handleOne(payload, clientId);
}
