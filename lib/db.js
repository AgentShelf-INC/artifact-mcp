...
export function seedKeysFromEnv(sha256Hex, raw = process.env.ARTIFACT_API_KEYS || "") {
  ...
  const upsert = db.prepare(`
    INSERT INTO api_keys (client_id, org, key_hash, revoked_at) VALUES (?, ?, ?, NULL)
    ON CONFLICT(client_id) DO UPDATE SET org = excluded.org, key_hash = excluded.key_hash, revoked_at = NULL
  `);
  let n = 0;
