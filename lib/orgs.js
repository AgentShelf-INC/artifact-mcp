...
const listOrgsStmt = db.prepare("SELECT name, label, created_at FROM orgs ORDER BY name ASC");
...
export function createOrg({ name, label, domain } = {}) {
  ...
  if (name.toLowerCase() === "admin") throw new Error('"admin" is a reserved org name.');
