// SPDX-License-Identifier: Apache-2.0
// Copyright 2026 Neil Blackman
// Resolve the viewing user's org from the Cloudflare Access identity.
//
// Cloudflare Access injects, on every request that passed the front door:
//   Cf-Access-Authenticated-User-Email : the user's email
//   Cf-Access-Jwt-Assertion            : a signed JWT proving it
//
// Production identity comes only from a verified JWT. Trusting the email header is an
// explicit local-development mode because a client that can reach the origin can forge it.
import { createRemoteJWKSet, jwtVerify } from "jose";
import { orgForDomain, orgForEmail as explicitOrgForEmail } from "./orgs.js";

const TEAM_DOMAIN = (process.env.CF_ACCESS_TEAM_DOMAIN || "").trim(); // e.g. yourteam.cloudflareaccess.com
const AUD = (process.env.CF_ACCESS_AUD || "").trim(); // Access application AUD tag
const TRUST_HEADERS = process.env.TRUST_ACCESS_HEADERS === "1";
// Cloudflare issues each Access JWT with `nbf`/`iat` at issuance time using its own accurate clock.
// If the origin's clock is a few seconds behind, a just-minted token is briefly "not yet valid"
// and verification throws `"nbf" claim timestamp check failed`, so the first post-login request
// 404s until the clock catches up (a manual reload then works). A small tolerance absorbs that
// unavoidable edge/origin skew without meaningfully weakening `exp`. Keep the host clock synced too.
const CLOCK_TOLERANCE_SECONDS = Number(process.env.ACCESS_CLOCK_TOLERANCE_S) || 60;

// domain -> org, e.g. "example.com:acme,team.example.org:teamb"
const DOMAIN_ORG = new Map(
  (process.env.ORG_EMAIL_DOMAINS || "")
    .split(",")
    .map((p) => p.split(":").map((s) => s.trim()))
    .filter(([d, o]) => d && o)
    .map(([d, o]) => [d.toLowerCase(), o])
);

// Admins see every org. Match by explicit email or by domain.
const ADMIN_EMAILS = new Set(
  (process.env.ADMIN_EMAILS || "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean)
);
const ADMIN_DOMAINS = new Set(
  (process.env.ADMIN_EMAIL_DOMAINS || "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean)
);

let jwks = null;
function getJwks() {
  if (!jwks && TEAM_DOMAIN) {
    jwks = createRemoteJWKSet(new URL(`https://${TEAM_DOMAIN}/cdn-cgi/access/certs`));
  }
  return jwks;
}

export const JWT_VERIFICATION_ON = Boolean(TEAM_DOMAIN && AUD);
export const ACCESS_IDENTITY_MODE = JWT_VERIFICATION_ON
  ? "jwt"
  : TRUST_HEADERS
    ? "header-trust"
    : "disabled";

export function assertReady() {
  if (process.env.REQUIRE_ACCESS_JWT === "1" && !JWT_VERIFICATION_ON) {
    throw new Error(
      "REQUIRE_ACCESS_JWT=1 requires both CF_ACCESS_TEAM_DOMAIN and CF_ACCESS_AUD; refusing to start"
    );
  }
  // Spoofable header-trust must not run on a publicly-reachable bind. Allow it only on a
  // loopback LISTEN_HOST, or when an operator explicitly acknowledges a proxy/compose that
  // controls host exposure (e.g. a same-network reverse proxy that injects the identity header).
  if (ACCESS_IDENTITY_MODE === "header-trust") {
    const host = (process.env.LISTEN_HOST || "0.0.0.0").trim();
    const loopback = host === "127.0.0.1" || host === "::1" || host === "localhost";
    if (!loopback && process.env.HEADER_TRUST_ALLOW_INSECURE !== "1") {
      throw new Error(
        `TRUST_ACCESS_HEADERS=1 trusts a spoofable identity header and is unsafe on a non-loopback ` +
        `bind (${host}). Set LISTEN_HOST=127.0.0.1 for local dev, configure CF_ACCESS_* for ` +
        `production, or set HEADER_TRUST_ALLOW_INSECURE=1 only when a proxy controls host exposure.`
      );
    }
  }
}

function domainOf(email) {
  const at = email.lastIndexOf("@");
  return at >= 0 ? email.slice(at + 1).toLowerCase() : "";
}

function orgForEmail(email) {
  const domain = domainOf(email);
  const isAdmin = ADMIN_EMAILS.has(email.toLowerCase()) || ADMIN_DOMAINS.has(domain);
  if (isAdmin) return { org: "admin", isAdmin: true };
  // Explicit address membership handles contractors/shared providers without re-routing their
  // whole domain. Admin configuration stays authoritative and can never be demoted by a mapping.
  const org = explicitOrgForEmail(email) || orgForDomain(domain) || DOMAIN_ORG.get(domain) || domain;
  return { org, isAdmin: false };
}

// Read the signed Cloudflare Access session token without weakening its trust boundary.
// Cookie values may contain "=", so split only on the first separator and match the
// cookie name exactly. Verification remains the responsibility of resolveViewer.
export function readAccessCookie(req) {
  const cookie = req?.headers?.cookie;
  const values = Array.isArray(cookie) ? cookie : [cookie];
  for (const value of values) {
    if (!value) continue;
    for (const part of String(value).split(";")) {
      const separator = part.indexOf("=");
      if (separator < 0 || part.slice(0, separator).trim() !== "CF_Authorization") continue;
      const token = part.slice(separator + 1).trim();
      if (token) return token;
    }
  }
  return "";
}

// -> { email, org, isAdmin }. email/org null means "no access".
export function createViewerResolver({ verifyJwt = jwtVerify } = {}) {
  return async function resolveViewer(req) {
    let email = "";

    if (ACCESS_IDENTITY_MODE === "jwt") {
      const assertion = req.headers["cf-access-jwt-assertion"];
      const headerToken = Array.isArray(assertion) ? assertion[0] : assertion;
      const token = headerToken || readAccessCookie(req);
      if (!token) return { email: null, org: null, isAdmin: false };
      try {
        const { payload } = await verifyJwt(token, getJwks(), {
          issuer: `https://${TEAM_DOMAIN}`,
          audience: AUD,
          clockTolerance: CLOCK_TOLERANCE_SECONDS
        });
        email = String(payload.email || "").trim().toLowerCase();
      } catch {
        return { email: null, org: null, isAdmin: false };
      }
    } else if (ACCESS_IDENTITY_MODE === "header-trust") {
      const h = req.headers["cf-access-authenticated-user-email"];
      email = String((Array.isArray(h) ? h[0] : h) || "").trim().toLowerCase();
    } else {
      return { email: null, org: null, isAdmin: false };
    }

    if (!email) return { email: null, org: null, isAdmin: false };
    const { org, isAdmin } = orgForEmail(email);
    return { email, org, isAdmin };
  };
}

export const resolveViewer = createViewerResolver();
