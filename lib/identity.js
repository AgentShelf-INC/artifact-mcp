...
function orgForEmail(email) {
  ...
  // Default: the email domain IS the org, so any domain you allow in Cloudflare Access
  // auto-tenants with zero app changes. The map is an optional override for a pretty
  // org name or to merge several domains into one org.
  const org = DOMAIN_ORG.get(domain) || domain;
