...
const listStmt = db.prepare(
  "SELECT client_id, org, created_at, revoked_at FROM api_keys ORDER BY (revoked_at IS NOT NULL), org, client_id"
);
const existsStmt = db.prepare("SELECT 1 FROM api_keys WHERE client_id = ?");
const insertStmt = db.prepare("INSERT INTO api_keys (client_id, org, key_hash) VALUES (?, ?, ?)");
...
export function createKey({ clientId, org }) {
  ...
  insertStmt.run(clientId, org, sha256Hex(secret));
  return { clientId, org, secret };
}
